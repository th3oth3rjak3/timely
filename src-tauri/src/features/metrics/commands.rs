...
fn generate_date_range(start_date: &DateTime<Utc>, end_date: &DateTime<Utc>) -> Vec<DateTime<Utc>> {
    (0..)
...
async fn get_daily_work_history(
    ...
    for record in task_work_history.iter() {
        ...
        values.iter().for_each(|(id, day, seconds)| {
            seconds_per_task_per_day.upsert_add((*id, *day), *seconds)
        });
    }
    ...
    seconds_per_task_per_day
        .iter()
        .map(|((id, date), seconds)| DailyWorkHistory {
            task_id: *id,
            day_worked: *date,
