...
use crate::{
    features::{tags::Tag, tasks::TaskTag},
    query_utils::add_in_expression,
    Data, PagedData, SortDirection,
};
...
pub async fn add_new_tag(new_tag: String, db: State<'_, Data>) -> TAResult<Tag> {
    ...
    let maybe_tag: Option<Tag> = sqlx::query_as!(
        Tag,
        r#"
            SELECT *
            FROM tags
    ...
    match maybe_tag {
        ...
        None => {
            ...
            sqlx::query_as!(Tag, "SELECT * FROM tags WHERE id = ?", inserted_id)
                .fetch_one(&db.pool)
                .await
                .into_ta_result()
        }
...
#[tauri::command]
pub async fn remove_tag_from_task(task_id: i64, tag_id: i64, db: State<'_, Data>) -> TAResult<()> {
    sqlx::query!(
        r#"
...
#[tauri::command]
pub async fn add_tag_to_task(tag_id: i64, task_id: i64, db: State<'_, Data>) -> TAResult<()> {
